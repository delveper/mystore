...
func (p *Product) create(rw http.ResponseWriter, req *http.Request) {
	respond(rw, req, http.StatusInternalServerError, errors.New("create"))
}

func (p *Product) read(rw http.ResponseWriter, req *http.Request) {
	respond(rw, req, http.StatusInternalServerError, errors.New("read"))
}

func (p *Product) readAll(rw http.ResponseWriter, req *http.Request) {
	respond(rw, req, http.StatusInternalServerError, errors.New("read all"))
}

func (p *Product) update(rw http.ResponseWriter, req *http.Request) {
	respond(rw, req, http.StatusInternalServerError, errors.New("update"))
}

func (p *Product) delete(rw http.ResponseWriter, req *http.Request) {
	respond(rw, req, http.StatusInternalServerError, errors.New("delete"))
}
...
func (p *Product) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	if req.URL.Path == "" {
		...
		case http.MethodGet:
			p.readAll(rw, req)
		default:
			respond(rw, req, http.StatusMethodNotAllowed, nil)
		}
		return
	}
	...
	if err != nil {
		respond(rw, req, http.StatusNotFound, nil)
		return
	}
